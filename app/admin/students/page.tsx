...
export default function AdminStudents() {
  ...
  const fetchFilterOptions = async (state?: string) => {
    try {
      const url = state ? `/api/admin/students/filter-options?state=${encodeURIComponent(state)}` : "/api/admin/students/filter-options";
  ...
  useEffect(() => {
    if (!checkingAuth) {
      fetchFilterOptions(stateFilter || undefined);
      if (!stateFilter) {
        setCityFilter(""); // Clear city filter when state is cleared
      }
    }
  }, [stateFilter, checkingAuth]);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Search by name, phone, city, or Student ID (e.g., S-10)..."
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setCityFilter(""); // Clear city filter when state changes
                      setPage(1);
                    }}
                    className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#072F6B] focus:border-transparent"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#072F6B] focus:border-transparent disabled:bg-gray-100 disabled:cursor-not-allowed"
                  >
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Videos</th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">ID Card</th>
                      ...
                      {students.map((student) => (
                        <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-semibold text-[#072F6B]">{formatStudentId(student.id)}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">{student.full_name}</div>
                            <div className="text-sm text-gray-500">DOB: {new Date(student.date_of_birth).toLocaleDateString()}</div>
                          </td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {student.id_card_key ? (
                              <Button onClick={() => viewIdCard(student.id)} disabled={loadingIdCard} variant="outline" size="sm" className="gap-2">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                            ...
                            <div className="text-xs text-gray-500">{new Date(student.created_at).toLocaleTimeString()}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 py-1 text-xs font-semibold rounded-full ${student.status === "approved" ? "bg-green-100 text-green-800" : student.status === "rejected" ? "bg-red-100 text-red-800" : student.status === "reviewed" ? "bg-yellow-100 text-yellow-800" : "bg-blue-100 text-blue-800"}`}>{student.status}</span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <select value={student.status} onChange={(e) => updateStatus(student.id, e.target.value)} className="px-3 py-1.5 border border-gray-300 rounded-lg bg-white text-gray-900 text-xs focus:outline-none focus:ring-2 focus:ring-[#072F6B] focus:border-transparent">
                              <option value="pending">Pending</option>
                              <option value="submitted">Submitted</option>
                              <option value="reviewed">Reviewed</option>
                              <option value="approved">Approved</option>
                    ...
                    <div key={student.id} className="p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="text-sm font-semibold text-[#072F6B] mb-1">{formatStudentId(student.id)}</div>
                          <div className="text-sm font-semibold text-gray-900">{student.full_name}</div>
                          <div className="text-xs text-gray-500 mt-1">DOB: {new Date(student.date_of_birth).toLocaleDateString()}</div>
                          <div className="text-xs text-gray-600 mt-1">{student.phone}</div>
                          <div className="text-xs text-gray-500 mt-1">
                            {student.city}, {student.state}
                          </div>
                        </div>
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full whitespace-nowrap ${student.status === "approved" ? "bg-green-100 text-green-800" : student.status === "rejected" ? "bg-red-100 text-red-800" : student.status === "reviewed" ? "bg-yellow-100 text-yellow-800" : "bg-blue-100 text-blue-800"}`}>{student.status}</span>
                      </div>
                      ...
                      <div className="flex flex-col gap-2 pt-2 border-t border-gray-100">
                        ...
                        {student.id_card_key ? (
                          <Button onClick={() => viewIdCard(student.id)} disabled={loadingIdCard} variant="outline" size="sm" className="w-full gap-2">
                            <Eye className="h-4 w-4" />
                            View ID Card
                          </Button>
                        ) : (
                        ...
                        <select value={student.status} onChange={(e) => updateStatus(student.id, e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 text-xs focus:outline-none focus:ring-2 focus:ring-[#072F6B] focus:border-transparent">
                          <option value="pending">Pending</option>
                          <option value="submitted">Submitted</option>
                          <option value="reviewed">Reviewed</option>
                          <option value="approved">Approved</option>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-2 sm:p-4">
              <div className="relative w-full max-w-4xl h-[95vh] sm:h-[90vh] max-h-[800px] bg-white rounded-xl border-2 border-gray-200 shadow-2xl overflow-hidden flex flex-col">
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 border-b border-gray-200">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">ID Card - Student ID: {formatStudentId(selectedIdCard.studentId)}</h3>
                    <p className="text-sm text-gray-600">{selectedIdCard.fileName}</p>
                  </div>
                  <Button variant="ghost" size="icon" onClick={() => setSelectedIdCard(null)} className="h-8 w-8">
                    <X className="h-4 w-4" />
                  </Button>
                </div>
                ...
                <div className="relative bg-gray-100 flex-1 flex items-center justify-center overflow-auto p-4">
                  {selectedIdCard.signedUrl.endsWith(".pdf") ? (
                    <div className="w-full h-full flex items-center justify-center">
                      <iframe src={selectedIdCard.signedUrl} className="w-full h-full min-h-[600px] border-0 rounded-lg" title="ID Card PDF" />
